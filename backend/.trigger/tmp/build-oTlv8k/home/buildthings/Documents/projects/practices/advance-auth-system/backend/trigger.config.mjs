...
async function generateJWT(options) {
  const { SignJWT } = await import("../../../../../../../esm-TY52GVRE.mjs");
...
async function loadSuperJSON() {
  const superjson = await import("../../../../../../../dist-ZGIOVZOV.mjs");
...
function runShapeStream(url, options) {
  const abortController = new AbortController();
  const version1 = new SSEStreamSubscriptionFactory(getEnvVar("TRIGGER_STREAM_URL", getEnvVar("TRIGGER_API_URL")) ?? "https://api.trigger.dev", {
    headers: options?.headers,
    signal: abortController.signal
  });
  const version2 = new ElectricStreamSubscriptionFactory(getEnvVar("TRIGGER_STREAM_URL", getEnvVar("TRIGGER_API_URL")) ?? "https://api.trigger.dev", {
    headers: options?.headers,
    signal: abortController.signal
  });
  options?.signal?.addEventListener("abort", () => {
...
var SSEStreamSubscriptionFactory = class {
  ...
  createSubscription(metadata, runId, streamKey, baseUrl) {
    if (!runId || !streamKey) {
...
var ElectricStreamSubscription = class {
  url;
  options;
  constructor(url, options) {
    this.url = url;
  ...
  async subscribe() {
    return zodShapeStream(SubscribeRealtimeStreamChunkRawShape, this.url, this.options).stream.pipeThrough(new TransformStream({
      transform(chunk, controller) {
        controller.enqueue(chunk.value);
      }
    })).pipeThrough(new LineTransformStream()).pipeThrough(new TransformStream({
      transform(chunk, controller) {
...
var ElectricStreamSubscriptionFactory = class {
  baseUrl;
  options;
  constructor(baseUrl, options) {
    this.baseUrl = baseUrl;
  ...
  createSubscription(metadata, runId, streamKey, baseUrl) {
    ...
    return new ElectricStreamSubscription(`${baseUrl ?? this.baseUrl}/realtime/v2/streams/${runId}/${streamKey}`, this.options);
  }
};
var VersionedStreamSubscriptionFactory = class {
  version1;
  version2;
  ...
  createSubscription(metadata, runId, streamKey, baseUrl) {
    ...
    const version = typeof metadata.$$streamsVersion === "string" ? metadata.$$streamsVersion : "v1";
    const $baseUrl = typeof metadata.$$streamsBaseUrl === "string" ? metadata.$$streamsBaseUrl : baseUrl;
    if (version === "v1") {
        ...
        if (run.metadata && "$$streams" in run.metadata && Array.isArray(run.metadata.$$streams)) {
          for (const streamKey of run.metadata.$$streams) {
            ...
            if (!activeStreams.has(streamKey)) {
              ...
              const subscription = this.options.streamFactory.createSubscription(run.metadata, run.id, streamKey, this.options.client?.baseUrl);
...
var trigger_config_default = defineConfig({
  project: "proj_suhworwbyuxaddzinvkr",
